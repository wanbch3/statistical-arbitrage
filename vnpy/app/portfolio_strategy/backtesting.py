...
class BacktestingEngine:
    ...
    def calculate_statistics(self, df: DataFrame = None, output=True) -> None:
        ...
        #     self.output(f"胜率：\t{percentage_profitable:,.2f}%")    ##
        #
        ...
        #     self.output(f"平均交易周期：\t{average_trade_cycle}")      ##
        #
    ...
    def show_trading_sketch(self, df: DataFrame = None)-> None:##
        ...
        :return:
        """
        # print(len(self.dts_temp))
        # print(type(self.long_position_point))
        # print(len([a for a in self.long_position_point if a is None]))
        # print('long:',[a for a in self.long_position_point if a is not None])
        # print('close long:', [a for a in self.close_long_position_point if a is not None])
        # print('short:', [a for a in self.short_position_point if a is not None])
        # print('close short:', [a for a in self.close_short_position_point if a is not None])
        if df is None:
    ...
    def write_report(self, filename, setting, stat: Dict = None) -> None:
        ...
        # temp = filename + '#' + str(datetime.now()) + '.txt'
        f = open(temp, "w")
    ...
    def new_bars(self, dt: datetime) -> None:
        ...
        # self.strategy.on_bars(bars,True)

        # results=self.strategy.on_bars(bars,True)
        # print(results)
        # current_spread=results[0]
        # boll_up=results[1]
        # boll_mid=results[2]
        # boll_down=results[3]
        ...
        # try:
        #     current_spread, boll_up, boll_mid, boll_down=self.strategy.on_bars(bars,True)
        #     results = {'date': dt, "current_spread": current_spread, "boll_up": boll_up, "boll_mid": boll_mid,
        #                "boll_down": boll_down}
        #     temp_df = DataFrame(results, index=[0]).set_index("date")
        #     # temp_df = DataFrame.from_dict(results).set_index("date")
        #     self.spread_df = pd.concat([self.spread_df, temp_df])
        # except (TypeError):
        ...
        # results = {'date': dt,"current_spread":current_spread, "boll_up":boll_up,"boll_mid":boll_mid,"boll_down":boll_down}
        # temp_df = DataFrame.from_dict(results).set_index("date")
        # self.spread_df = pd.concat([self.spread_df, temp_df])
