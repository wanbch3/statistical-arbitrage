...
class EWMA(StrategyTemplate):
    ...
    def __init__(
        ...
        self.spread_data: np.array = np.zeros(self.boll_window)
    ...
    def on_bars(self, bars: Dict[str, BarData],bool = False):
        ...
        buf: np.array = self.spread_data
        buf = pd.DataFrame(buf)

        self.boll_mid=buf.ewm(alpha=self.Alpha).mean().iloc[-1,0]
        self.boll_up=self.boll_mid+ self.entry_level
        self.boll_down = self.boll_mid - self.entry_level
