...
class bwsArbitrageStrategy(StrategyTemplate):
    ...
    def on_bars(self, bars: Dict[str, BarData],bool = False):
        ...
        B = self.boll_mid / sig_square_n + self.prior_mean / self.prior_variance
        self.posterior_mean = B / A
        self.posterior_variance = (1 / A) ** (0.5)
        # self.boll_down = norm.ppf(self.boll_mid, loc=self.posterior_mean, scale=self.posterior_variance)
        # self.boll_up = norm.ppf(1 - self.boll_mid, loc=self.posterior_mean, scale=self.posterior_variance)
        ...
        if not leg1_pos:
            if self.current_spread >= self.boll_up:
                ...
                self.long_position_point=self.boll_up
                ...
                self.short_position_point = self.boll_down
                self.targets[self.leg1_symbol] = 1
                self.targets[self.leg2_symbol] = -1
        elif leg1_pos > 0:
